                                            ...
                                            <div className="mb-3">
                                                ...
                                                <div className="space-y-1 text-xs text-[#6C6C6C] font-inter">
                                                    <div className="flex justify-between">
                                                        <span>{pricingDetails.city_matched ? `${property.ville} (${pricingDetails.is_major_city ? __('Major City') : __('City')})` : __('Default Price')}</span>
                                                        <span>{formatPrice(pricingDetails.base_price)}</span>
                                                    </div>
                                                    {!pricingDetails.is_flat_rate && pricingDetails.price_adjustment > 0 && (
                                                        <div className="flex justify-between">
                                                            <span>{pricingDetails.tier_name} {__('Adjustment')}</span>
                                                            <span>+{formatPrice(pricingDetails.price_adjustment)}</span>
                                                        </div>
                                                    )}
                                                    {pricingDetails.is_flat_rate && (
                                                        <div className="flex justify-between">
                                                            <span>{pricingDetails.tier_name} ({__('Flat Rate')})</span>
                                                            <span>{formatPrice(price)}</span>
                                                        </div>
                                                    )}
                                                </div>
                                                <div className="mt-2 pt-2 border-t border-[#EAEAEA]">
                                                    <span className="px-2 py-1 text-xs rounded bg-[#BFDBFE] text-[#1E40AF]">
                                                        {__('Tier')}: {__(pricingDetails.tier_name)}
                                                    </span>
                                                </div>
                                            </div>
                                        )}
